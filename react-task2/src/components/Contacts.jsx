...
const Contacts = () => {
    ...
    const [searchTerm, setSearchTerm] = useState("");
    const [errors, setErrors] = useState({ name: "", lastName: "", email: "", phone: "" });
    ...
    const validateContact = (contact) => {
        ...
        if (!contact.lastName.trim()) error.lastName = "Last name is required";
        if (!contact.email) error.email = "Email is required";
        else if (!/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}$/i.test(contact.email)) error.email = "Email Address is invalid";
        if (!contact.phone) error.phone = "Phone number is required";
        else if (contact.phone.length < 11) error.phone = "Phone number must be at least 11 digits";
        return error;
    };
        ...
        <>
            ...
            <div className={styles.inp}>
                ...
                {selectedContacts.length > 0 && (
                    <button className={styles.deleteBtn} onClick={selectDeleteHandler} disabled={selectedContacts.length === 0}>
                        Select
                    </button>
                )}
            </div>
